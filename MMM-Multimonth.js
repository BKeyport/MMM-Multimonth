...
const EventHelper = {
    ...
    getEventsForDate(date) {
        ...
        return this.events.filter(
            (ev) => this.dayKey(Number(ev.startDate)) === target && this.matchCalendar(ev.calendarName)
        );
    },
    ...
    getEventCountsForDate(date) {
        ...
        for (let ev of this.events) {
            if (this.dayKey(Number(ev.startDate)) !== target || !this.matchCalendar(ev.calendarName)) continue;
...
Module.register("MMM-Multimonth", {
    ...
    resolveConfig() {
        ...
        // New Routine.
        if (typeof c.staticStartMonth === "number" && c.staticStartMonth >= 1 && c.staticStartMonth <= 12) {
        ...
        if (cfg.dowHeader !== "short" && cfg.dowHeader !== "narrow" && cfg.dowHeader !== "long") {
            cfg.dowHeader = "short";
        }
        ...
        if (Array.isArray(c.weekendDays) && c.weekendDays.length) {
            cfg.weekendDays = [
                ...new Set(c.weekendDays.map(Number).filter((day) => Number.isInteger(day) && day >= 0 && day <= 6)),
            ];
        } else {
            const w1 = Number(c.weekend1);
            const w2 = Number(c.weekend2);

            cfg.weekendDays = [w1, w2].filter((day) => Number.isInteger(day) && day >= 0 && day <= 6);
        ...
        if (Number.isInteger(cfg.startWeek) && cfg.startWeek >= 0 && cfg.startWeek <= 6) {
            // Valid user override, keep as-is
        } else {
            // Use locale default
            const locale = new Intl.Locale(config.language);
        ...
        } else if (typeof c.weekNumbers === "number" && c.weekNumbers >= 1 && c.weekNumbers <= 4) {
            cfg.weekNumbers = true;
        ...
        cfg.calNames = Array.isArray(c.calNames) ? [...c.calNames] : [];
    ...
    buildSnapshot(cfg) {
        ...
        switch (cfg.staticMode) {
            case "relative":
                startDate = new Date(today.getFullYear(), today.getMonth() + cfg.startMonth, 1);
                break;

            default:
                startDate = new Date(today.getFullYear(), cfg.startMonth - 1, 1);
        }
            ...
            while (current <= displayEnd) {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    const day = {
                        ...
                        today: current.toDateString() === today.toDateString(),

                        weekend: cfg.highlightWeekend && cfg.weekendDays.includes(current.getDay()),
    ...
    renderCalendar(snapshot, cfg) {
        ...
        wrapper.classList.add(cfg.monthsVertical ? "vertical" : "horizontal");
        ...
        snapshot.months.forEach((month, monthIndex) => {
            ...
            if (!cfg.monthsVertical || (cfg.monthsVertical && (monthIndex === 0 || cfg.repeatDOW))) {
            ...
            month.weeks.forEach((week) => {
                monthDiv.appendChild(this.renderWeek(week, cfg));
            });
    ...
    renderDOWHeader(cfg) {
        ...
        if (cfg.weekNumbers) {
            ...
            blank.className = cfg.bigCalendar ? "weekNumBig dowBlank" : "weekNumSmall dowBlank";
    ...
    renderWeek(week, cfg) {
        ...
        if (cfg.weekNumbers) {
            ...
            weekCell.className = cfg.bigCalendar ? "weekNumBig" : "weekNumSmall";
        ...
        week.days.forEach((day) => {
            row.appendChild(this.renderDay(day, cfg));
        });
    ...
    renderDay(day, cfg) {
        ...
        const eventData = this.eventHelper ? this.eventHelper.getEventsForDate(day.date) : [];
        ...
        const data = this.eventHelper ? this.eventHelper.getEventCountsForDate(day.date) : {};
        ...
        if (cfg.bigCalendar) {
            ...
            for (const name in data) {
                ...
                if (cfg.eventsCount) {
                    e.innerHTML = `<span style="color:${color}" class="icon ${symbol}"> x ${data[name].count}</span>`;
                } else {
                    e.innerHTML = `<span style="color:${color}" class="icon ${symbol}"></span>`;
                }
